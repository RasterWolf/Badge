...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 col2 = vec3(0);
    float sum=0.;
	//int AngleNum = int(iTime - (floor(iTime / 4.0) * 4.0))+1;
    ...
    for(int i=0;i<AngleNum;i++)
    {
        ...
        for(int j=0;j<SampNum;j++)
        {
            ...
            float s=3.5;
            	...
            	g=getGrad(pos2,fract(iTime/10.0));
    ...
    //fragColor += 0.077;
    //fragColor = min(fragColor , 5.5*texture2D (iChannel0 , fragCoord/iResolution.xy));
